...
function getOrCreateSheet(ss, sheetName, headers) {
  ...
  function loadFromStorage() {
    try {
      ...
      if (!localStorage.getItem('khmer_wheel_demo_cleared_v3')) {
        ...
        localStorage.setItem('khmer_wheel_demo_cleared_v3', 'true');
      }
  ...
  function downloadSampleExcel() {
    const sampleData = [
      { 'ល.រ': 1, 'ឈ្មោះសិស្ស': '', 'ថ្នាក់': '', 'ភេទ': '', 'សម្គាល់': '' },
      { 'ល.រ': 2, 'ឈ្មោះសិស្ស': '', 'ថ្នាក់': '', 'ភេទ': '', 'សម្គាល់': '' }
    ];
  ...
  function init() {
    ...
    // ទាញយកទិន្នន័យពី Google Sheet ដោយស្វ័យប្រវត្តិ ប្រសិនបើបើកតាមរយៈ Shared Link
    if (hasSharedSheet && googleSheetUrl && googleSheetUrl.startsWith('https://script.google.com/')) {
